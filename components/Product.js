...
function Product({ title, price, description, category, image, rating }) {
  ...
  return (
    <div
      data-aos="fade-up"
      className="flex flex-col min-h-[374px] min-w-[321px] bg-white shadow-lg hover:bg-slate-100 px-2 py-1 m-5 cursor-pointer"
    >
      <div
        onClick={() => {
          setLiked(!liked);
      ...
      >
        {!liked ? (
          <HeartIconOutline className="h-7 cursor-pointer float-right" />
        ) : (
          <HeartIconSolid className="h-7 cursor-pointer text-red-500 float-right" />
        )}
      </div>
      <div className="relative h-[229px] w-[321px] bg-white  mx-auto ">
        <Image
          src={image}
          layout="fill"
          objectFit="contain"
